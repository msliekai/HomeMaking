...
<%--                <s:property value="list"></s:property>--%>
                <div class="layui-card-header">
                    <button class="layui-btn" onclick="xadmin.open('添加','<%=path%>page/manger/AddTritem.jsp',600,400)"><i class="layui-icon"></i>添加</button>
                </div>
        ...
        table.on('tool(test)', function(obj) {
            ...
            if (obj.event === 'tdel') {
                layer.confirm('确定删除？', function (index) {
                    fal("<%=path%>manager/delTritem.action",data.itid);
                    layer.close(index);
                });
                ...
                layer.open({
                    ...
                    title: "技能培训修改",
                    area: ['450px', '430px'],
                    content: "UpdateTritem.jsp"+
                        "?itid="+encodeURIComponent(data.itid)+
                        "&itname="+encodeURIComponent(data.itname)+
                        "&ittime="+encodeURIComponent(data.ittime)+
                        "&itbook="+encodeURIComponent(data.itbook)
